...
public class CommonUtil {
     ...
     * @Description 生成支付订单号
     **/
    public static String createOrderNum(Product product) {
        String outTradeNo = product.getOutTradeNo();
        String payType = product.getPayType();
        String sysid = "1";//默认掌柜智囊
        ...
        outTradeNo = outTradeNo.replace(".", "_");
        String res = outTradeNo + "_" + payType + "_" + i + "_" + sysid;
        return res;
    }
    ...
    public static String convertMessage(String message) {
        ...
        if ("PAYING".equals(message)) {//随行付支付订单下单成功
            msg = "new";
        } else if ("SUCCESS".equals(message)) {//随行付支付订单成功
            msg = "success";
        }else if ("REFUNDSUC".equals(message)) {//随行付退款成功结果
            msg = "success";
        }else if ("REFUNDFAIL".equals(message)) {//随行付退款失败结果
            msg = "fail";
        }
        return msg;
    }
...
//		String num = createOrderNum("123456789.123","1","1");
//		System.out.println(num);
        System.out.println(convertMessage("success"));
    }
}